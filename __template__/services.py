...
class TemplateService:
    ...
    def create_template(self, data: Dict[str, Any]) -> Template:
        ...
        if existing:
            raise ValueError(f"Template with name '{validated_data['name']}' already exists")
    ...
    def update_template(self, template_id: int, data: Dict[str, Any]) -> Optional[Template]:
    ...
    def list_templates(self, page: int = 1, per_page: int = 10, 
                      filters: Optional[Dict[str, Any]] = None) -> Tuple[List[Template], int]:
    ...
    def search_templates(self, search_params: Dict[str, Any]) -> Tuple[List[Template], int]:
    ...
    def get_template_stats(self) -> Dict[str, Any]:
        ...
        active_templates = self.session.query(Template).filter_by(is_active=True).count()
        ...
        for (category,) in categories:
            if category:
                count = self.session.query(Template).filter_by(category=category).count()
        ...
        # Most used templates
        most_used = self.session.query(Template).order_by(desc(Template.usage_count)).limit(5).all()
    ...
    def import_template(self, import_data: Dict[str, Any]) -> Template:
        ...
        template_data = {
            key: value for key, value in import_data.items()
            if key not in ['id', 'uuid', 'created_at', 'updated_at', 'export_info', 'usage_count', 'last_used_at']
        }
...
class TemplateAnalyticsService:
    ...
    def get_usage_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        templates = self.session.query(Template).filter(
            and_(
    ...
    def get_category_analysis(self) -> Dict[str, Any]:
        ...
        for (category,) in categories:
            if category:
                templates = self.session.query(Template).filter_by(category=category).all()
                analysis[category] = {
                    'count': len(templates),

...
def get_template(template_id):
    ...
    try:
        ...
        error_response = error_schema.dump(
            {"error": "internal_server_error", "message": "Failed to retrieve template"}
        )
        return jsonify(error_response), 500
...
def create_template():
    ...
    try:
        ...
        error_response = error_schema.dump(
            {"error": "internal_server_error", "message": "Failed to create template"}
        )
        return jsonify(error_response), 500
...
def update_template(template_id):
    ...
    try:
        ...
        logger.warning(
            f"Validation error updating template {template_id}: {e.messages}"
        )
        error_response = error_schema.dump(
            {
                "error": "validation_error",
        ...
        error_response = error_schema.dump(
            {"error": "internal_server_error", "message": "Failed to update template"}
        )
        return jsonify(error_response), 500
...
def delete_template(template_id):
    ...
    try:
        ...
        # Serialize response
        result = success_schema.dump({"message": "Template deleted successfully"})
        ...
        error_response = error_schema.dump(
            {"error": "internal_server_error", "message": "Failed to delete template"}
        )
        return jsonify(error_response), 500
...
def health_check():
    ...
    try:
        ...
        error_response = error_schema.dump(
            {"error": "service_unavailable", "message": "Service health check failed"}
        )

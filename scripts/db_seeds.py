...
def generate_sample_posts(count: int = 50, user_count: int = 10) -> List[Dict[str, Any]]:
    ...
    categories = ['Technology', 'Science', 'Health', 'Travel', 'Food', 'Sports', 'Entertainment']
    ...
    for i in range(count):
        post = {
            ...
            'published_at': fake.date_time_between(start_date='-3m', end_date='now') if random.random() > 0.3 else None,
            'tags': random.sample(['python', 'flask', 'api', 'web', 'backend', 'frontend', 'database', 'ml', 'ai'], k=random.randint(1, 4))
        }
        posts.append(post)
...
def generate_sample_orders(count: int = 100, user_count: int = 10, product_count: int = 30) -> List[Dict[str, Any]]:
...
def generate_api_logs(count: int = 200) -> List[Dict[str, Any]]:
    ...
    endpoints = ['/api/users', '/api/posts', '/api/products', '/api/orders', '/health', '/api/status']
...
def seed_posts(count: int = 20) -> None:
    ...
    for i in range(count):
        ...
        slug = title.lower().replace(' ', '-').replace('.', '').replace(',', '').replace('!', '').replace('?', '') + f'-{i}'
        ...
        post = Post(
            ...
            published_at=fake.date_time_between(start_date='-30d', end_date='now') if is_published else None
        )
        db.session.add(post)

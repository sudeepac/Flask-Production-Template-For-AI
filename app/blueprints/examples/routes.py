...
def health_check():
    ...
    try:
        ...
        logger.error(
            f"Health check endpoint error: {e}", extra={"error_type": type(e).__name__}
        )
        return (
            jsonify(
                {
...
def create_user_advanced():
    ...
    try:
        # Log request start
        logger.info(
            "Advanced user creation started",
            extra={
        ...
        if existing_user:
            conflict_field = (
                "username" if existing_user.username == username else "email"
            )
            logger.warning(
        ...
        logger.error(
            f"Unexpected error in advanced user creation: {e}",
            extra={"error_type": type(e).__name__, "ip_address": request.remote_addr},
        )
...
def create_post_for_user(user_id: int):
    ...
    try:
        ...
        if not user:
            logger.warning(
                f"Attempted to create post for non-existent user",
        ...
        try:
            ...
            logger.warning(
                "Post schema validation failed",
                extra={"validation_errors": e.messages, "user_id": user_id},
            )
        ...
        logger.error(
            f"Unexpected error creating post: {e}",
            extra={"user_id": user_id, "error_type": type(e).__name__},
        )
        raise APIError("Failed to create post")

...
examples_index_model = examples_ns.model(
    "ExamplesIndex",
    {
...
health_check_model = examples_ns.model(
    "HealthCheck",
    {
        "status": fields.String(
            required=True, description="Overall health status", example="healthy"
        ),
        "timestamp": fields.String(
            required=True,
...
user_create_request_model = examples_ns.model(
    "UserCreateRequest",
    {
        ...
        "full_name": fields.String(
            required=False, description="Full name of the user", example="John Doe"
        ),
        "age": fields.Integer(
            required=False,
...
user_create_response_model = examples_ns.model(
    "UserCreateResponse",
    {
        "message": fields.String(
            required=True,
...
post_create_request_model = examples_ns.model(
    "PostCreateRequest",
    {
        "title": fields.String(
            required=True,
...
post_create_response_model = examples_ns.model(
    "PostCreateResponse",
    {
        "message": fields.String(
            required=True,
...
error_simulation_response_model = examples_ns.model(
    "ErrorSimulationResponse",
    {
        ...
        "timestamp": fields.String(
            required=True, description="Error timestamp", example="2024-01-01T12:00:00Z"
        ),
    },
)

performance_test_response_model = examples_ns.model(
    "PerformanceTestResponse",
    {
        ...
        "test_results": fields.Raw(
            ...
            example={
                ...
                "memory_usage": {"before_mb": 45.2, "after_mb": 47.8, "peak_mb": 52.1},
            },
        ),
...
class AdvancedUserCreateResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logger.info(
                "User created with advanced validation",
...
class PostCreateResource(Resource):
    ...
    def post(self, user_id):
        ...
        try:
            ...
            logger.info(
                "Post created for user", extra={"user_id": user_id, "post_title": title}
            )
...
class ErrorSimulationResource(Resource):
    ...
    def get(self, error_type):
        ...
        if error_type not in error_responses:
            examples_ns.abort(
                400,
                f"Unknown error type: {error_type}. Supported types: validation, not_found, rate_limit, server_error",
            )
...
class PerformanceTestResource(Resource):
    ...
    @examples_ns.response(
        200, "Performance test completed", performance_test_response_model
    )
    @log_performance
    def get(self):

...
class BaseSchema(Schema):
    ...
    def _enhance_error_messages(self, errors: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field, messages in errors.items():
            if isinstance(messages, list):
                enhanced[field] = [
                    f"Field '{field}': {msg}" for msg in messages
                ]
    ...
    @classmethod
    def create_response_schema(cls, data_schema: Type['BaseSchema']) -> Type['BaseSchema']:
...
class MetadataMixin:
    ...
    server_timestamp = fields.DateTime(dump_only=True, format='iso', default=datetime.utcnow)